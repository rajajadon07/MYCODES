...
if choice=="USA":
    ...
    for i in search(query,tld='co.in',country=result ,lang='en',num=5,stop=5,pause=2,tbs="2"):
          req=requests.get(i)
    ...
    result="CHINA"
    query=st.text_input("Please enter your query here")
    for i in search(query,tld='co.in',country=result ,lang='en',num=5,stop=5,pause=2,tbs="qdr:m"):
          req=requests.get(i)
          soup = BeautifulSoup(req.text, 'html.parser')
          st.write("Title of this website: " )
          for title in soup.find_all('title',limit=1):
...
if option <2 and option < 3:
      for i in search(query,tld='co.in',lang='en',num=1,stop=1,pause=2,tbs="qdr:m"):
         result1=st.write(i)
      ...
      for i in search(query,tld='co.in',lang='en',num=2,stop=2,pause=2,tbs="qdr:m"):
         temp=3
         count=2
         result2=st.write(i)
      if count<temp:
     ...
     for i in search(query,tld='co.in',lang='en',num=3,stop=3,pause=2,tbs="qdr:m"):
        temp=4
        count=3
        result3=st.write(i)
        if count<temp:
