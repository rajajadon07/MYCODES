...
def top_headlines():
    ...
    top_headlines
    # %%
    get_everything(query, language,  from_param,
                   to, sort_by='relevancy')
    # %%
    import datetime
    ...
    def func(query):
        ...
        for (from_dt, to_dt) in zip(from_list, to_list):
            all_articles = newsapi.get_everything(
                q=query, language='en', sort_by='relevancy',  from_param=from_dt, to=to_dt)
    ...
    def wordcld(dictionary):
        ...
        for (k, v) in dictionary.items():
            if v != '[]':
                ...
                print(str(k[0])+"_"+str(k[1][5:10])+"_"+str(k[1][11:13]) +
                      "_"+str(k[1][14:16]) + "_"+str(k[1][17:19])+"_"+str(k[2]))
                wordcloud.to_file("wordcloud_images/" + (str(k[0])+"_"+str(k[1][5:10])+"_"+str(
                    k[1][11:13]) + "_"+str(k[1][14:16]) + "_"+str(k[1][17:19]))+"_"+".png")
            else:
                print(str(k[0])+"_"+str(k[1][5:10])+"_"+str(k[1][11:13]) +
                      "_"+str(k[1][14:16]) + "_"+str(k[1][17:19])+"_"+str(k[2]))
                print("Wordcloud Not applicable")
    wordcld(dic)
    st.code(top_headlines, language="python")
